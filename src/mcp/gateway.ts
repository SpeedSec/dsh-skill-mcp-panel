...
import {
  ...
  toolRemovePayloadSchema,
  toolSavePayloadSchema,
  toolSetEnabledPayloadSchema,
  toolTestPayloadSchema
} from "./wire.js";
import { fiberPhaseOf, getLoaderEntry, mcpToolCount, waitForLoaderState } from "./status.js";
import { probeMcpServer } from "./probe.js";
import { modesFor, policyRow, removeRule, replaceRule, rulesFromRows, type ModeRule } from "../mode-policy.js";
...
export class McpManagerGateway extends TypertRemoteService {
  ...
  withRules(rows: PatchRow[], rules: ModeRule[]): PatchRow[] {
    const next = rows.filter((row) => row.id !== MODE_POLICY_ROW_ID);
    const policy = policyRow(rules);
    if (policy !== undefined) next.push(policy);
    return next;
  }

  async modes() {
    const presets = await this.C.agentPresets?.list?.() ?? [];
    const modes = presets.map((preset: any) => stripUndefined({
      id: preset.id,
    ...
    if (presets.some((preset: any) => preset.id === "security-research")) {
      modes.push({
  ...
  async list() {
    ...
    try {
      ...
      for (const row of managedMcp) {
        ...
        if (view !== undefined) servers.push({ ...view, modes: modesFor(rules, "mcp", view.serverName) });
      }
  ...
  async saveTool(rawPayload: unknown) {
    ...
    if (externalTools.some((row) => row.config?.toolName === payload.input.toolName)) throw new Error(`toolName ${JSON.stringify(payload.input.toolName)} 已被外部 adapter 行占用`);
    if (managedTools.some((row) => row.config?.toolName === payload.input.toolName && row.config?.toolName !== previousName)) throw new Error(`toolName ${JSON.stringify(payload.input.toolName)} 已存在`);
    const previous = managedTools.find((row) => toolNameFromRowId(row.id) === previousName || row.config?.toolName === previousName);
    if (payload.previousToolName !== undefined && previous === undefined) throw new Error(`要编辑的适配工具不存在：${JSON.stringify(previousName)}`);
  ...
  async removeTool(rawPayload: unknown) {
    ...
    if (row === undefined) throw new Error(`适配工具 ${JSON.stringify(payload.toolName)} 不存在或不是面板受管行`);
  ...
  async setToolEnabled(rawPayload: unknown) {
  ...
  async testTool(rawPayload: unknown) {
    const payload = toolTestPayloadSchema.parse(rawPayload);
    if ("input" in payload) return probeSubprocessTool(this.C, payload.input, payload.target);
    ...
    if (row === undefined) throw new Error(`适配工具 ${JSON.stringify(payload.toolName)} 不存在`);
