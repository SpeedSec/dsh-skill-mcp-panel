...
export async function probeSubprocessTool(ctx: any, rawInput: unknown, target: string) {
  let input;
  try {
  ...
  if (target.trim() === "") return { ok: false, target, exitCode: null, stdout: "", stderr: "", error: "测试 target 不能为空" };
