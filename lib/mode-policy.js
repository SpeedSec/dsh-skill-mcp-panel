...
import { MODE_POLICY_PLUGIN_NAME, MODE_POLICY_ROW_ID } from "./patch-editor.js";
export const name = "skill-mcp-mode-policy";
export const inject = ["tools", "agents", "agentPresets"];
function selectedTarget(ctx, agent) {
...
export function rulesFromRows(rows) {
    ...
    return validRules({ rules: Array.isArray(row?.config?.rules) ? row.config.rules : [] });
}
export function policyRow(rules) {
...
export function apply(ctx, config) {
    ...
    const managedNames = () => {
        const names = ctx.tools.schemas().map((schema) => schema.name).filter((value) => typeof value === "string");
        return names.filter((toolName) => rules.some((rule) => matches(rule, toolName)));
    };
    const reconcile = (agent) => {
        ...
        const denied = managedNames().filter((toolName) => rules.some((rule) => matches(rule, toolName) && !allows(rule, target)));
        if (denied.length > 0)
            restrictions.set(agent, agent.ctx.tools.restrict({ deny: denied }));
    };
    const reconcileAll = () => {
        if (stopping || reconciling)
            return;
        reconciling = true;
        try {
    ...
    ctx.on("security-research/worker-tools", ({ tools }) => {
        for (const rule of rules) {
            if (!rule.modes.includes("security-research-workers"))
                continue;
            for (const schema of ctx.tools.schemas()) {
                if (typeof schema?.name === "string" && matches(rule, schema.name))
                    tools.add(schema.name);
            }
        }
    });
    reconcileAll();
