...
import { z } from "zod";
import { MANAGED_TOOL_ROW_ID_PREFIX, SUBPROCESS_ADAPTER_PLUGIN_NAME } from "../patch-editor.js";
...
export function toolRowId(toolName) {
    return MANAGED_TOOL_ROW_ID_PREFIX + toolName;
}
export function toolNameFromRowId(id) {
    if (typeof id !== "string" || !id.startsWith(MANAGED_TOOL_ROW_ID_PREFIX))
        return undefined;
    const value = id.slice(MANAGED_TOOL_ROW_ID_PREFIX.length);
    return TOOL_NAME_RE.test(value) ? value : undefined;
}
export function toToolPatchRow(input, enabled = true) {
    return {
...
export function toolInputFromRow(row) {
    if (row.name !== SUBPROCESS_ADAPTER_PLUGIN_NAME)
        throw new Error("不是 subprocess adapter 行");
    return subprocessToolInputSchema.parse(row.config ?? {});
}
export function toolRowToView(row) {
    const input = toolInputFromRow(row);
    return { ...input, enabled: row.disabled !== true, entryId: row.id };
}
export function applyToolEdit(previous, input, enabled = true) {
    if (previous === undefined)
        return toToolPatchRow(input, enabled);
