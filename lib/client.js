    ...
    factory: (require) => {
        ...
        function ToolManager({ t, data, refresh, saveTool, removeTool, setToolEnabled, testTool }) {
            ...
            const save = async (input, previousToolName, selectedModes) => {
                ...
                setError(null);
                try {
                    await saveTool(input, previousToolName, selectedModes);
                    setEditing(null);
                    await refresh();
                }
                catch (failure) {
            ...
            const toggle = async (tool) => { try {
                await setToolEnabled(tool.toolName, !tool.enabled);
                await refresh();
            }
            catch (failure) {
                setError(String(failure?.message ?? failure));
            ...
            const remove = async (tool) => {
                if (confirming !== tool.toolName)
                    return setConfirming(tool.toolName);
                setConfirming(null);
                try {
            ...
            const test = async (tool) => {
                ...
                if (target === null || target.trim() === "")
                    return;
                setTesting(tool.toolName);
                setResult(null);
                try {
                    setResult({ toolName: tool.toolName, value: await testTool(tool.toolName, target.trim()) });
                }
                catch (failure) {
                    setResult({ toolName: tool.toolName, value: { ok: false, error: String(failure?.message ?? failure), stdout: "", stderr: "", exitCode: null } });
                }
                finally {
                    setTesting(null);
                }
